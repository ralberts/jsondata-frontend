  ...
  return gulp.src('./src/app/index.html')
    ...
    .pipe(replace({
      patterns: [
        {
          match: 'backendUrl',
          replacement: process.env.OPENSHIFT_NODE4_PORT ? "http://jsondatabackend-ryanalberts.rhcloud.com:8000" : settings.backendUrl
        }
      ]
    }))
    .pipe(gulp.dest('./.tmp/'))
...
gulp.task('statics', g.serve({
  port: process.env.OPENSHIFT_NODE4_PORT || settings.frontend.ports.development,
  hostname: process.env.OPENSHIFT_NODE4_IP || '127.0.0.1',
...
gulp.task('production', g.serve({
  port: process.env.OPENSHIFT_NODE4_PORT || settings.frontend.ports.production,
  hostname: process.env.OPENSHIFT_NODE4_IP || '127.0.0.1',
  root: ['./dist'],
  middleware: historyApiFallback({})
}));
